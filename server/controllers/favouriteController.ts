...
export const FavouriteController = {
   ...
   */
  getFavourites: async (req: NextRequest, session: Session, logger: Logger) => {
    const favlogger = logger.child({
      module: 'FavouriteController',
      op: 'getFavourites',
    })
    const type = req.nextUrl.searchParams.get('type')

    if (!type || !session.user.id) {
      favlogger.warn('Type Missing')
      return Response.json(
        {
          success: false,
          message: 'Valid session and Favourite Type required',
        },
        { status: 400 },
      )
    }
