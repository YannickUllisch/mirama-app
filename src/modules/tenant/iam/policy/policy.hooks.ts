...
const policy = {
  ...
  fetchAllForScope: {
    useQuery: (scope: AccessScope) => {
      ...
      return useQuery<PolicyResponse[]>({
        queryKey: [
          ...policyKeys.tenant(activeTenantId),
          'allForScope',
          scope,
        ] as const,
        queryFn: async () => {
          const res = await fetchPoliciesFn(activeTenantId, scope, {
            pageNumber: 1,
            pageSize: 100,
          })
          return res.items
        ...
        ...optimisticList<PolicyResponse, Vars>(
          ...
          policyKeys.list(activeTenantId),
          {
            ...
            apply: (old, { id, data }) =>
              old.map((p) => {
                if (p.id !== id) return p
                const afterRemoves = p.statements.filter(
                  (s) => !data.removeStatementIds.includes(s.id),
                )
                const afterAdds = [
                  ...afterRemoves,
                  ...data.addStatements.map((s) => ({
