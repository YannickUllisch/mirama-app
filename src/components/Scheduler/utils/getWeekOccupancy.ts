...
export const getWeekOccupancy = (
  ...
  const getHoursAndMinutes: TimeUnits[] = occupancy.map((item) => {
    ...
    const { hours: itemHours, minutes: itemMinutes } = getDuration(
      item.occupancy,
    )
    ...
    if (focusedWeek === endWeekNum) {
      const hours =
        endDateDayNum > businessDays
          ? businessDays * itemHours
          : endDateDayNum * itemHours
      const minutes =
        endDateDayNum > businessDays
          ? businessDays * itemMinutes
          : endDateDayNum * itemMinutes
      return { hours, minutes }
    }

    if (dayjs(focusedDate).isBetween(item.startDate, item.endDate)) {
      return {
        hours: businessDays * itemHours,
        minutes: businessDays * itemMinutes,
      }
    }
  ...
  const { hours: totalHours, minutes: totalMinutes } =
    getTotalHoursAndMinutes(getHoursAndMinutes)
