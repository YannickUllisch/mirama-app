...
export function useDataTable<TData>(props: UseDataTableProps<TData>) {
  ...
  const [, setAdvancedFilters] = useQueryState(
    filtersKey,
    ...
    (updaterOrValue: Updater<ColumnFiltersState>) => {
      if (enableAdvancedFilter) {
        setColumnFilters((prev) => {
          ...
          const extFilters = next
            .filter((f) => f.value !== '' && f.value !== null)
            .map((f): ExtendedColumnFilter<TData> => {
              const col = columns.find((c) => c.id === f.id)
              const variant = (col?.meta?.variant ?? 'text') as FilterVariant
              const val = f.value as string | string[]
              return {
                id: f.id as Extract<keyof TData, string>,
                value: val,
                variant,
                operator: (Array.isArray(val)
                  ? 'inArray'
                  : 'iLike') as FilterOperator,
                filterId: f.id,
              }
            })

          void setAdvancedFilters(extFilters)
          return next
        })
        return
      }
